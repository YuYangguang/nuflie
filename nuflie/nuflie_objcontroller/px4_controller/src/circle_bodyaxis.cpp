...
/*void angleCountReceived(std_msgs::Int32 count)
{
    angleCount[1]=count.data;
}

void angleCountReceived2(std_msgs::Int32 count)
{
...
void WorldModelReceived(nuflie_common::WorldModelMsg worldInfo)
{
    int i=0;
    int otherUAVCount=1;
    for(i=1;i<UAV_NUM+1;i++)   //UAV ID is arranged from 1
    {
        if(i!=UAV_NUM)
        {
            angleCount[otherUAVCount]=worldInfo.consensusinfo[i].angle.data;
            otherUAVCount++;
        }
    }
}
int main(int argc, char **argv)
{
  ...
  angleCountPublisher=nodeHandle.advertise<std_msgs::Int32>("angleCount",10);
  ...
  //ros::Subscriber angleCountSubscirber2=nodeHandle.subscribe("/nuflie02/angleCount",10,angleCountReceived2);
  ros::Subscriber worldModelSubscirber=nodeHandle.subscribe("worldmodel/worldmodelinfo",10,WorldModelReceived);
